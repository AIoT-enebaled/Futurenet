...
import { getFirestore, doc, setDoc, serverTimestamp, Timestamp, collection, query, orderBy, limit, onSnapshot, addDoc, enableNetwork, disableNetwork, getDoc, where, getDocs } from "firebase/firestore";
...
class CustomUser {
  ...
  constructor(userProfile: User) { // Constructor now takes the fetched User profile
...
class AuthService {
  async signIn(email: string, password: string): Promise<CustomUser> {
    ...
    if (!userProfile) {
       ...
       console.error("User profile not found for authenticated user:", credential.user.uid);
  ...
  async signUp(email: string, password: string, displayName: string, role: User['role']): Promise<CustomUser> {
    ...
    const initialRoles: UserRole = {
        reader: true, // All users can read by default
        author: role === 'instructor' || role === 'admin', // Instructors and Admins can be authors
        admin: role === 'admin' // Only Admins are admins
    };
    ...
    const newUserProfile: User = {
        ...
        role: role,
        roles: initialRoles,
        createdAt: serverTimestamp() as Timestamp, // Server timestamp for creation
        updatedAt: serverTimestamp() as Timestamp // Server timestamp for update
    ...
    const createdProfile = await this.getUserProfile(uid);
     if (!createdProfile) {
         // This case should ideally not happen if createUserProfile succeeds
  ...
  async getUserProfile(uid: string): Promise<User | null> {
    ...
    if (userDoc.exists()) {
      const data = userDoc.data();
       // Ensure all required properties are present and have correct types
      if (data && typeof data.email === 'string' && typeof data.displayName === 'string' &&
          typeof data.role === 'string' && data.createdAt instanceof Timestamp && data.updatedAt instanceof Timestamp) {
        return {
          uid: uid,
          email: data.email,
        ...
        return null; // Or throw an error to indicate data inconsistency
      }
      ...
      if (user) {
        ...
        if (userProfile) {
          ...
          callback(new CustomUser(userProfile));
           ...
           console.error("Authenticated user profile not found in Firestore for UID:", user.uid);
...
class FileUploadService {
  ...
  async uploadAssignmentSubmission(assignmentId: string, userId: string, file: File): Promise<string> {
    const fileName = `${Date.now()}_${file.name}`;
...
class DiscussionService {
  subscribeToMessages(discussionId: string, callback: (messages: any[]) => void) {
  ...
  async sendMessage(discussionId: string, userId: string, content: string): Promise<void> {
    const messagesRef = collection(db, 'discussions', discussionId, 'messages');
...
class CloudFunctionsService {
  ...
  private enrollStudent = httpsCallable<EnrollmentRequest, EnrollmentResponse>(functions, 'enrollStudent');
...
class CourseService {
  ...
  async getUserCourses(userId: string): Promise<Course[]> {
    ...
    const coursePromises = enrollments.docs.map(doc =>
      this.getCourse(doc.data().courseId) // Reusing getCourse, which uses caching
    );
...
export { AuthService, CustomUser, FileUploadService, DiscussionService, OfflineService, CloudFunctionsService, CourseService, Timestamp };  export type { User, UserRole, Course, Lesson, Assignment };
