...
const ChatBotPage: React.FC<ChatBotPageProps> = ({
  ...
  const [groundingMetadatas, setGroundingMetadatas] = useState<
    Record<string, GroundingMetadata | undefined>
  >({});
  ...
  const handleSendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const { text: botText, groundingMetadata } = await generateBotResponse(
          text,
          historyForGemini,
        );
        ...
        if (groundingMetadata) {
          setGroundingMetadatas((prev) => ({
            ...prev,
            [botMessage.id]: groundingMetadata,
          }));
        }
      } catch (err) {
        const errorMessage =
  ...
  if (apiKeyExists === null) {
    return (
      <div className="flex justify-center items-center h-full">
        <LoadingSpinner message="Initializing AI Assistant..." />
      </div>
    );
  }
  ...
  if (!currentUser && apiKeyExists) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] text-center p-8 bg-brand-surface rounded-xl shadow-xl border border-brand-border/30">
        <MessageSquareIcon className="w-16 h-16 text-brand-purple mb-6" />
        <h1 className="text-2xl font-bold text-brand-text mb-3">
          GiiT AI Assistant
        </h1>
        <p className="text-brand-text-muted mb-6 max-w-md">
          Please log in to start chatting with our AI assistant. It's free for
          all users!
        </p>
        <Link
          to="/login"
          ...
          <div className="flex justify-start mb-4">
            <div className="flex items-end gap-2.5">
              <img
                src={BOT_AVATAR_URL}
                alt="bot avatar"
                className="w-7 h-7 rounded-full border-2 border-brand-surface object-cover"
              />
      ...
      {!apiKeyExists && (
        <div className="p-3 bg-yellow-900/40 border-t border-yellow-700/40">
          <p className="text-yellow-300 text-sm text-center">
            AI features are currently unavailable (API Key missing). Admins are
            on it! 🛠️
          </p>
        </div>
      )}
      {!currentUser &&
        apiKeyExists && ( // This case is less likely now due to page-level block
          <div className="p-3 bg-brand-purple/20 border-t border-brand-purple/40 text-center">
            <p className="text-brand-text text-sm">
              Please{" "}
              <Link
                to="/login"
                className="font-semibold hover:underline text-brand-cyan"
              >
                log in
              </Link>{" "}
              or{" "}
