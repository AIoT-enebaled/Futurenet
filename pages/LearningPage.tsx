...
import {
  ...
  User,
  LearningPageProps,
  CourseCategory as CourseCategoryType,
  CourseDifficulty,
} from "../types";
...
const LearningPage: React.FC<LearningPageProps> = ({
  ...
  const filteredCourses = useMemo(() => {
    return courses.filter((course) => {
      ...
      const matchesCategory =
        !categoryFilter || course.categoryIds.includes(categoryFilter);
  ...
  const handlePurchaseComplete = (courseId: string, paymentMethod: string) => {
    onPurchaseCourse(courseId, false);
    alert(
      `Course purchased successfully using ${paymentMethod}! You now have access to all course content.`,
    );
  };

  const commonButtonStyles =
    "px-3 py-1.5 text-xs sm:px-4 sm:py-2 sm:text-sm rounded-lg font-medium transition-all duration-200 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand-surface";
    ...
    <div className="space-y-8">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-brand-text-muted text-sm sm:text-base">
            Expand your knowledge with courses curated by GiiT experts.
          </p>
        </div>
      </div>
      ...
      <div className="md:hidden">
        ...
        >
          ...
          <ChevronDownIcon
            className={`w-5 h-5 ml-auto transform transition-transform ${showFilters ? "rotate-180" : ""}`}
          />
        </button>
      </div>
      ...
      {/* Filter Section */}
      <div
        className={`${showFilters ? "block" : "hidden"} md:block p-4 bg-brand-surface rounded-xl border border-brand-border/30 shadow-lg space-y-4`}
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setDifficultyFilter(
                  (e.target.value as CourseDifficulty) || null,
                )
              }
              className="w-full p-2.5 border border-brand-border bg-brand-bg text-brand-text rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-brand-purple focus:border-brand-purple text-sm transition-colors"
            >
      ...
      {filteredCourses.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-xl font-semibold text-brand-text-muted">
            No courses match your current filters.
          </p>
          <p className="text-sm text-brand-text-darker mt-2">
