...
const InstructorDashboardPage: React.FC<InstructorDashboardPageProps> = ({
  currentUser,
  courses,
  instructors,
  onCreateCourse,
  ...
  const [newCourseData, setNewCourseData] = useState<
    Omit<Course, "id" | "rating" | "reviewsCount" | "reviews">
  >({
    title: "",
    subtitle: "",
    description: "",
  ...
  const handleInputChange = (
    ...
    >,
    field: keyof typeof newCourseData,
  ) => {
    const value = e.target.value;
    setNewCourseData((prev) => ({
      ...prev,
      [field]:
        field === "durationHours" || field === "price"
          ? parseFloat(value) || 0
          : value,
    }));
  };
  ...
  const addLesson = (moduleIndex: number) => {
    setNewCourseData((prev) => {
      ...
      targetModule.lessons = [
        ...
        {
          id: `less-${Date.now()}`,
          title: `New Lesson ${targetModule.lessons.length + 1}`,
          durationMinutes: 10,
        },
      ];
      return { ...prev, modules: updatedModules };
  ...
  const handleCreateCourseSubmit = (e: React.FormEvent) => {
    ...
    if (
      ...
      newCourseData.modules.length === 0
    ) {
      alert(
        "Please fill in all required fields: Title, Description, at least one Category, and at least one Module.",
      );
      return;
    }
    const coursePayload = {
      ...newCourseData,
      imageUrl:
        newCourseData.imageUrl ||
        `https://picsum.photos/seed/course-${Date.now()}/400/225`,
    };
    if (courseToEdit) {
  ...
  const handleDeleteCourse = (courseId: string) => {
    if (
      window.confirm(
        "Are you sure you want to delete this course? This is a mock action.",
      )
    ) {
  ...
  const secondaryButtonStyles =
    "flex items-center justify-center px-4 py-2 text-sm font-medium text-brand-text-muted bg-brand-surface-alt hover:bg-opacity-80 rounded-lg shadow-sm border border-brand-border hover:border-brand-purple/50 transition-colors focus:outline-none focus:ring-2 focus:ring-brand-purple focus:ring-offset-2 focus:ring-offset-brand-surface";
  const tabButtonStyles =
    "w-full flex items-center px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ease-in-out group";
  const activeTabButtonStyles =
    "bg-gradient-to-r from-brand-purple to-brand-pink text-white shadow-md";
  const inactiveTabButtonStyles =
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-brand-text-muted text-sm sm:text-base">
            Manage your courses, create new content, and view analytics.
          </p>
        </div>
      </div>
        ...
        <aside className="md:w-1/4 lg:w-1/5 flex-shrink-0">
          <nav className="space-y-1.5 sticky top-20">
            ...
            <button
              ...
              className={`${tabButtonStyles} ${activeTab === "my-courses" ? activeTabButtonStyles : inactiveTabButtonStyles}`}
            >
              <ListTreeIcon className="w-5 h-5 mr-3" />
              My Courses
            </button>
            <button
              onClick={() => {
              ...
              className={`${tabButtonStyles} ${inactiveTabButtonStyles}`}
            >
                ...
                {instructorCourses.length > 0 ? (
                  <div className="space-y-4">
                    {instructorCourses.map((course) => (
                      <div
                        key={course.id}
                        className="p-4 bg-brand-bg rounded-lg border border-brand-border/40 flex justify-between items-center hover:border-brand-purple/50 transition-colors"
                      >
                        <div>
                  ...
                  <p className="text-brand-text-muted">
                    You haven't created any courses yet. Click "Create Course"
                    to get started!
                  </p>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-brand-bg rounded-lg border border-brand-border/40">
                    ...
                    <p className="text-3xl font-bold text-brand-text">
                      4.7{" "}
                      <StarIcon className="w-6 h-6 inline text-yellow-400 mb-1" />
                    </p>
                  </div>
      ...
      <Modal
        ...
        title={
          courseToEdit
            ? `Edit Course: ${courseToEdit.title.substring(0, 25)}...`
            : "Create New Course"
        }
        size="2xl"
      >
        <form
          ...
          className="space-y-4 max-h-[75vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-brand-border scrollbar-track-transparent"
        >
          <div>
            <label
              htmlFor="courseTitle"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="number"
                id="courseDuration"
                value={newCourseData.durationHours}
                onChange={(e) => handleInputChange(e, "durationHours")}
                required
                min="0"
                step="0.5"
                className={commonInputStyles}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-brand-text-muted mb-0.5">
              ...
              <div className="max-h-28 overflow-y-auto p-2 border border-brand-border rounded-md bg-brand-bg space-y-1">
                {COURSE_CATEGORIES_DATA.map((cat) => (
                  <label
                    key={cat.id}
                    className="flex items-center space-x-2 cursor-pointer p-1 hover:bg-brand-surface-alt rounded"
                  >
                    <input
                      type="checkbox"
                      checked={newCourseData.categoryIds.includes(cat.id)}
          ...
          <div>
            ...
            <input
              ...
              className={commonInputStyles}
              placeholder="https://picsum.photos/seed/mycourse/400/225"
            />
          </div>
          <div>
            <label
              htmlFor="coursePreviewVideoUrl"
              className="block text-sm font-medium text-brand-text-muted mb-0.5"
            >
            ...
            <input
              type="url"
              id="coursePreviewVideoUrl"
              value={newCourseData.previewVideoUrl}
              onChange={(e) => handleInputChange(e, "previewVideoUrl")}
              className={commonInputStyles}
          ...
          <div>
            ...
            <input
              type="text"
              id="courseTags"
              value={newCourseData.tags?.join(", ")}
              onChange={handleTagChange}
              className={commonInputStyles}
              placeholder="e.g. python, web dev, ai"
            />
          </div>
          <div className="flex items-center pt-2">
            <input
              id="coursePro"
              type="checkbox"
              checked={newCourseData.isProCourse}
              onChange={(e) =>
                setNewCourseData((prev) => ({
                  ...prev,
                  isProCourse: e.target.checked,
                }))
              }
            ...
            <label
              htmlFor="coursePro"
              className="ml-2 block text-sm text-brand-text cursor-pointer"
            >
              Pro Course (Requires subscription or direct purchase)
            </label>
          </div>

          {/* Modules and Lessons */}
          <h3 className="text-md font-semibold text-brand-text pt-3 border-t border-brand-border/30">
            Curriculum*
          </h3>
          {newCourseData.modules.map((module, moduleIndex) => (
            <div
              key={module.id || moduleIndex}
              className="p-3 bg-brand-bg/70 rounded-md border border-brand-border/50 space-y-2"
            >
              <div className="flex justify-between items-center">
                <input
                  ...
                  className={`${commonInputStyles} text-sm font-medium flex-grow mr-2`}
                />
                <button
                  type="button"
                  onClick={() => removeModule(moduleIndex)}
                  className="p-1 text-red-500 hover:text-red-400 rounded-full hover:bg-brand-surface-alt"
                  title="Remove Module"
                >
                  <Trash2Icon className="w-4 h-4" />
                </button>
              ...
              {module.lessons.map((lesson, lessonIndex) => (
                <div
                  ...
                  className="ml-4 p-2 bg-brand-surface/50 rounded border border-brand-border/30 space-y-1.5"
                >
                  <div className="flex justify-between items-center">
                    <input
                      type="text"
                      ...
                      className={`${commonInputStyles} text-xs flex-grow mr-2 py-1`}
                    />
                    <button
                      type="button"
                      onClick={() => removeLesson(moduleIndex, lessonIndex)}
                      className="p-1 text-red-500 hover:text-red-400 rounded-full hover:bg-brand-bg"
                      title="Remove Lesson"
                    >
                  ...
                  <input
                    ...
                    min="1"
                    className={`${commonInputStyles} text-xs py-1`}
                  />
                  <input
                    type="url"
                  ...
                  <label className="flex items-center space-x-2 text-xs text-brand-text-muted cursor-pointer">
                    <input
                      ...
                      className="h-3.5 w-3.5 text-brand-purple bg-brand-bg border-brand-border rounded focus:ring-brand-purple focus:ring-offset-brand-bg"
                    />
          ...
          <button
            type="button"
            onClick={addModule}
            className="mt-2 text-sm text-green-500 hover:text-green-400 hover:underline font-medium"
          >
            + Add Module
          </button>
