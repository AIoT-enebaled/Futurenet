  ...
  const currentMessengerUser: MessengerUser | null = currentUser
    ? MOCK_MESSENGER_USERS.find((u) => u.id === currentUser.id) || {
        ...
        avatarUrl:
          ...
          `https://ui-avatars.com/api/?name=${encodeURIComponent(currentUser.displayName)}&background=random&color=fff&size=40`,
        isOnline: true,
      }
    : null;
      ...
      if (chatIndex !== -1) {
        ...
        if ((foundChat.unreadCount || 0) > 0) {
          setChats((prevChats) => {
            ...
            if (
              newChats[chatIndex] &&
              newChats[chatIndex].id === activeChatIdFromParams
            ) {
        ...
        prevChats.map((c) =>
          c.id === chatId
            ? {
                ...
                typing: c.typing?.filter((t) => t.userId !== currentUser.id),
              }
            : c,
        ),
      );
              ...
              setChats((prevChats) =>
                prevChats.map((c) =>
                  c.id === chatId
                    ? {
                        ...c,
                        lastMessage: mockReply,
                        unreadCount:
                          c.id === activeChat?.id
                            ? 0
                            : (c.unreadCount || 0) + 1,
                        typing: c.typing?.filter(
                          (t) => t.userId !== randomParticipant.id,
                        ),
                      }
                    : c,
                ),
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] text-center p-8 bg-brand-surface rounded-xl shadow-xl border border-brand-border/30">
        ...
        <p className="text-brand-text-muted mb-6">
          Please log in to use the GiiT Messenger.
        </p>
        <Link
          to="/login"
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] max-h-[calc(100vh-4rem)] bg-brand-surface text-brand-text overflow-hidden rounded-xl shadow-2xl border border-brand-border/30">
      {(!isMobileView || (isMobileView && !activeChatIdFromParams)) && (
        <aside
          className={`w-full md:w-80 lg:w-96 flex-shrink-0 border-r border-brand-border/40 flex flex-col bg-brand-surface ${isMobileView && activeChatIdFromParams ? "hidden" : "flex"}`}
        >
      ...
      {(!isMobileView || (isMobileView && !!activeChatIdFromParams)) && (
        <main
          className={`flex-1 flex flex-col bg-brand-bg ${isMobileView && !activeChatIdFromParams ? "hidden" : "flex"}`}
        >
          <MessengerChatView
            chat={activeChat}
