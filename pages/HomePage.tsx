...
const PricingCard: React.FC<PricingCardProps> = ({
  ...
  const price =
    isAnnual && tier.priceAnnually ? tier.priceAnnually : tier.priceMonthly;
  return (
    <div
      className={`bg-brand-surface p-6 rounded-xl shadow-2xl border border-brand-border/30 flex flex-col ${tier.isPopular ? "border-brand-purple shadow-glow-purple relative ring-2 ring-brand-purple" : "hover:shadow-glow-cyan hover:border-brand-cyan/40 transform hover:-translate-y-1 transition-all duration-300"}`}
    >
      {tier.isPopular && (
      ...
      <ul className="space-y-2.5 mb-8 flex-grow">
        {tier.features.map((feature, index) => (
          <li
            key={index}
            className="flex items-start text-sm text-brand-text-muted"
          >
      ...
      <button
        ...
        className={`w-full mt-auto py-3 px-6 font-semibold rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand-surface
          ${
            tier.isPopular
              ? "bg-gradient-purple-pink text-white hover:shadow-glow-pink focus:ring-brand-purple"
...
const HomePage: React.FC<HomePageProps> = ({
  ...
  const [selectedTierForModal, setSelectedTierForModal] =
    useState<SubscriptionTier>("pro_individual");
  ...
  const handlePricingCtaClick = (
    tierId: SubscriptionTier | "contact_sales",
  ) => {
    if (tierId === "free") {
      ...
      alert(
        "Mock: You would be redirected to a sales contact form or Calendly link for Team Accelerator plan.",
      );
    }
  };
    ...
    <div className="space-y-20 pb-10">
      ...
      <div className="text-center py-12 px-6 md:py-20 md:px-10 bg-brand-surface rounded-2xl shadow-2xl border border-brand-border/20">
        ...
        <p className="text-lg md:text-xl text-brand-text-muted mb-10 max-w-3xl mx-auto leading-relaxed">
          Connect, collaborate, and innovate with the brightest minds at the
          Genius Institute of Information Technology. Explore new horizons, chat
          with our AI, and build the future, together.
        </p>
        ...
        <div className="mt-16 md:mt-20 p-6 md:p-8 bg-brand-bg/50 rounded-xl border border-brand-border/30 shadow-inner">
          ...
          <p className="text-brand-text-muted md:text-lg leading-relaxed">
            The Genius Institute of Information Technology (GiiT) is at the
            forefront of tech education, dedicated to nurturing innovation and
            cultivating excellence. This FutureNet platform is your dynamic
            space to interact with peers, access cutting-edge resources, and
            engage with our intelligent AI assistant for insights and support.
          </p>
        </div>
      </div>
      ...
      <section id="pricing" className="py-12 px-4 sm:px-6">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-brand-text mb-4">
            Flexible Plans for Everyone
          </h2>
          <p className="text-lg text-brand-text-muted max-w-2xl mx-auto">
            Choose the GiiT FutureNet plan that best suits your needs, from
            exploring freely to unlocking powerful Pro features.
          </p>
          <div className="mt-8 flex justify-center items-center space-x-2">
            <span
              className={`font-medium ${!isAnnualPricing ? "text-brand-purple" : "text-brand-text-muted"}`}
            >
              Monthly
            </span>
            <button
            ...
            >
              ...
              <span
                aria-hidden="true"
                className={`inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 ${isAnnualPricing ? "translate-x-5" : "translate-x-0"}`}
              ></span>
            </button>
            <span
              className={`font-medium ${isAnnualPricing ? "text-brand-purple" : "text-brand-text-muted"}`}
