...
interface PostCardProps {
  ...
  onAddReaction: (
    postId: string,
    reactionType: "heart" | "thumbsUp" | "fire" | "clap",
  ) => void;
      ...
      <div className="p-5 sm:p-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-2">
          <Link to={`/blog/${post.id}`} title={`Read more about ${post.title}`}>
            <h2 className="text-xl sm:text-2xl font-bold text-brand-text hover:text-brand-cyan transition-colors cursor-pointer line-clamp-2">
              {post.title}
            </h2>
          </Link>
          {isOwner && (
            <div className="flex items-center space-x-1.5 flex-shrink-0 ml-2">
              <button
                onClick={() => onEditPostRedirect(post.id)}
                title="Edit Post"
                className="p-1.5 rounded-full hover:bg-brand-surface-alt text-brand-text-muted hover:text-brand-cyan transition-colors"
              >
              ...
              <button
                ...
                title="Delete Post"
                className="p-1.5 rounded-full hover:bg-brand-surface-alt text-brand-text-muted hover:text-brand-pink transition-colors"
              >
        ...
        {post.subtitle && (
          <p className="text-sm sm:text-md text-brand-text-muted mb-3 line-clamp-2">
            {post.subtitle}
          </p>
        )}
        <p className="text-brand-text text-sm mb-4 leading-relaxed line-clamp-3 flex-grow">
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="px-2.5 py-0.5 bg-brand-bg text-brand-text-muted rounded-full text-xs font-medium border border-brand-border/40"
              >
                #{tag}
              </span>
            ))}
        ...
        <div className="mt-auto pt-4 border-t border-brand-border/20">
          <div className="flex flex-wrap justify-between items-center mb-3 text-xs text-brand-text-darker">
            ...
            <div className="flex items-center space-x-3 mt-2 sm:mt-0">
              ...
              >
                <HeartIcon
                  className={`w-4 h-4 mr-1 ${currentUser && post.reactions.heart ? "fill-brand-pink text-brand-pink" : "text-brand-text-darker group-hover:text-brand-pink"}`}
                />{" "}
              ...
              >
                <ThumbsUpIcon
                  className={`w-4 h-4 mr-1 ${currentUser && post.reactions.thumbsUp ? "fill-brand-cyan text-brand-cyan" : "text-brand-text-darker group-hover:text-brand-cyan"}`}
                />{" "}
              ...
              <Link
                ...
                title="View Comments"
                className="flex items-center text-brand-text-muted hover:text-brand-purple transition-colors"
              >
          ...
          <Link
            ...
            className="text-sm text-brand-purple hover:text-brand-pink font-semibold transition-colors inline-block"
          >
            Read more &rarr;
          </Link>
        </div>
...
export const BlogPage: React.FC<ExtendedBlogPageProps> = ({
  currentUser,
  posts,
  onCreatePost,
  onDeletePost,
  ...
  const [newPostData, setNewPostData] = useState<
    Omit<Post, "id" | "createdAt" | "reactions" | "comments">
  >({
    title: "",
    subtitle: "",
    body: "",
  ...
  useEffect(() => {
    if (currentUser && isCreateModalOpen) {
      ...
      })); // Ensure imageUrl is initialized
    }
    ...
    if (newPostData.title && newPostData.body) {
      ...
      const newPost: Post = {
        ...
        imageUrl:
          ...
          `https://picsum.photos/seed/blog-${newPostData.title.replace(/\s+/g, "-")}/600/400`,
      };
  ...
  const handleGenerateWithAIClick = async (
    fieldToUpdate: "body" | "title" | "image",
  ) => {
    ...
    if (fieldToUpdate === "image") {
      const imagePrompt = prompt(
        ...
        newPostData.title || "A futuristic technology concept",
      );
      if (imagePrompt) {
    ...
    alert(
      `Mock AI generated ${fieldToUpdate}! Actual text generation can be implemented using geminiService.`,
    );
  };

  const commonInputStyles =
    "mt-1 block w-full px-3.5 py-2.5 bg-brand-bg border border-brand-border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-purple focus:border-brand-purple sm:text-sm text-brand-text placeholder-brand-text-muted transition-colors";
  const primaryButtonStyles =
    "flex items-center justify-center bg-gradient-purple-pink text-white font-semibold py-2.5 px-5 rounded-lg shadow-lg hover:shadow-glow-pink transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-brand-purple focus:ring-offset-2 focus:ring-offset-brand-surface";
  const secondaryButtonStyles =
    "flex items-center justify-center bg-brand-surface text-brand-text-muted font-semibold py-2.5 px-5 rounded-lg shadow-md hover:bg-brand-surface-alt hover:text-brand-text border border-brand-border/70 hover:border-brand-purple/70 transition-colors focus:outline-none focus:ring-2 focus:ring-brand-purple focus:ring-offset-2 focus:ring-offset-brand-surface";
  const aiButtonStyles =
    "text-xs inline-flex items-center text-brand-cyan hover:text-brand-pink transition-colors ml-2 px-2 py-1 rounded-md border border-brand-cyan/50 hover:border-brand-pink/50 bg-brand-surface-alt/50 disabled:opacity-50 disabled:cursor-not-allowed";
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-brand-text-muted text-sm sm:text-base">
              News, articles, and updates from the GiiT community pulse.
            </p>
          </div>
        </div>
        {currentUser ? (
      ...
      <Modal
        ...
        title="Compose New Insight"
        size="xl"
      >
        <form onSubmit={handleCreatePostSubmit} className="space-y-4">
          <div>
            <label
              htmlFor="postTitleModal"
              className="block text-sm font-medium text-brand-text-muted mb-0.5"
            >
              Title
            </label>
            <div className="flex items-center">
              ...
              <button
                type="button"
                onClick={() => handleGenerateWithAIClick("title")}
                className={aiButtonStyles}
                title="Generate Title with AI (Pro)"
              >
          ...
          <div>
            ...
            <input
              type="text"
              id="postSubtitleModal"
              value={newPostData.subtitle}
              onChange={(e) =>
                setNewPostData({ ...newPostData, subtitle: e.target.value })
              }
              className={commonInputStyles}
          ...
          <div>
            ...
            <textarea
              ...
              rows={6}
              required
              className={`${commonInputStyles} min-h-[150px]`}
              placeholder="Share your thoughts..."
            ></textarea>
            <button
              type="button"
              onClick={() => handleGenerateWithAIClick("body")}
              className={`${aiButtonStyles} mt-1`}
              title="Generate Body with AI (Pro)"
            >
              <StarIcon className="w-3 h-3 mr-1 text-yellow-400" /> Generate
              with AI
            </button>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                placeholder="Paste URL or generate with AI"
                className={commonInputStyles}
              />
              <button
                type="button"
                onClick={() => handleGenerateWithAIClick("image")}
                className={`${aiButtonStyles} whitespace-nowrap py-2.5`}
                title="Generate Image with AI (Pro)"
            ...
            {newPostData.imageUrl && !isGeneratingImage && (
              <div className="mt-2 p-2 border border-brand-border rounded-md bg-brand-bg max-w-xs">
                ...
                <img
                  ...
                  alt="Post preview"
                  className="rounded max-h-40 w-auto object-contain"
                />
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-3">
            <button
              ...
              className={`${secondaryButtonStyles} py-2 px-4 text-sm`}
            >
              Cancel
            </button>
            <button
              ...
              className={`${primaryButtonStyles} py-2 px-4 text-sm`}
              disabled={isGeneratingImage}
            >
              Create Post
            </button>
      ...
      {localPosts.length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-brand-text-darker mt-1">
            {currentUser
              ? "Be the first to share one!"
              : "Log in to publish an insight."}
          </p>
        </div>
      )}
      <GoProModal
