    ...
    if (userDoc.exists()) {
       const data = userDoc.data();
        if (data && typeof data.email === 'string' && typeof data.displayName === 'string' &&
           typeof data.role === 'string' && data.createdAt instanceof Timestamp && data.updatedAt instanceof Timestamp && typeof data.roles === 'object') {
          return {
            uid: uid,
            email: data.email,
...
export class FileUploadService {
  ...
  async uploadAssignmentSubmission(assignmentId: string, userId: string, file: File) {
    const fileName = `${Date.now()}_${file.name}`;
...
export { AuthService, CustomUser, User, UserRole, Course, Lesson, Assignment, FileUploadService, DiscussionService, OfflineService, CloudFunctionsService, CourseService, Timestamp };