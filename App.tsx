...
const App: React.FC = () => {
  ...
  const [appSettings, setAppSettings] = useState<AppSettings>(() => {
    ...
    return storedSettings
      ...
      : { theme: "dark", notificationSounds: true };
  });
  ...
  const handleLogin = (user: User) => {
    const userToLogin: User = {
      ...user,
      is_pro_user:
        user.email === "geniusinstitute2024@gmail.com"
          ? true
          : user.is_pro_user || false,
      pro_expiry_date:
        user.email === "geniusinstitute2024@gmail.com"
          ? new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString()
          : user.pro_expiry_date || null,
      subscribed_tier:
        user.email === "geniusinstitute2024@gmail.com"
          ? "pro_individual"
          : user.subscribed_tier || "free",
      role:
        user.email === "geniusinstitute2024@gmail.com"
          ? "admin"
          : user.email === "instructor.drcode@giit.com"
            ? "instructor"
            : user.role || "member",
      instructorId:
        user.email === "instructor.drcode@giit.com"
  ...
  const handleRegister = (userWithPassword: User) => {
    ...
    const registeredUsers: User[] = storedUsersString
    ...
    const userToRegister: User = {
      ...
      role:
        userWithPassword.email === "instructor.drcode@giit.com"
          ? "instructor"
          : userWithPassword.role || "member",
      instructorId:
        userWithPassword.email === "instructor.drcode@giit.com"
          ? "instr-dr-code"
          : undefined,
  ...
  const handleUpdateUser = (
    ...
    if (storedUsersString) {
      ...
      if (userIndex > -1) {
        ...
        if (existingPassword)
          registeredUsers[userIndex].password = existingPassword;
        localStorage.setItem(
          "giitRegisteredUsers",
          JSON.stringify(registeredUsers),
        );
  ...
  const handleCreateTeam = (
    ...
    const newTeam: Team = {
      ...
      channels: [{ id: `gen-${Date.now()}`, name: "general", type: "text" }],
      files: [],
    };
    setTeams((prev) => [newTeam, ...prev]);
  };
  const handleUpdateTeam = (teamId: string, updatedData: Partial<Team>) =>
    setTeams((prev) =>
      prev.map((t) =>
        t.id === teamId
          ? {
              ...t,
              ...updatedData,
              membersCount: updatedData.members?.length || t.membersCount,
            }
          : t,
      ),
    );
  ...
  const handleCreatePost = (
    ...
    const newPost: Post = {
      id: `post-${Date.now()}`,
      ...postData,
      author: postData.author || currentUser.displayName,
      createdAt: new Date().toISOString(),
  ...
  const handleCreateProject = (
    ...
    const newProject: Project = {
      id: `proj-${Date.now()}`,
      ...projectData,
      owner: projectData.owner || currentUser.displayName,
      createdAt: new Date().toISOString(),
    };
  ...
  const handlePurchaseCourse = (courseId: string, isProCourse?: boolean) => {
    ...
    if (course.isProCourse && !currentUser.is_pro_user) {
      ...
      if (currentUser.is_pro_user || !course.isProCourse) {
        // re-check needed if modal was used
  ...
  const handleCreateCourseByInstructor = (
    newCourseData: Omit<Course, "id" | "rating" | "reviewsCount" | "reviews">,
  ) => {
      ...
      <Routes>
        ...
        >
          <Route
            ...
            element={
              <Navigate
                to={NAV_ITEMS.find((item) => item.isHome)?.path || "/home"}
                replace
              />
            }
          />
          ...
          <Route
            path="/events"
            element={<EventsPage currentUser={currentUser} />}
          />
            ...
            element={
              currentUser?.role === "instructor" ? (
                <InstructorDashboardPage
                  ...
                  onCreateCourse={handleCreateCourseByInstructor}
                  onUpdateCourse={handleUpdateCourse}
                />
              ) : (
          ...
          <Route
            ...
            element={
              !currentUser ? (
                ...
                <Navigate
                  to={
                    currentUser.role === "admin" && currentUser.is_pro_user
                      ? "/analytics"
                      : "/profile"
                  }
                  replace
