...
const GlobalChatWidget: React.FC<GlobalChatWidgetProps> = ({
  currentUser,
  apiKeyExists,
  onClose,
  onSubscribePro,
  ...
  return (
    <div className="fixed bottom-24 right-6 w-full max-w-sm h-[60vh] max-h-[480px] bg-brand-surface rounded-xl shadow-2xl flex flex-col z-50 border border-brand-border/50 transition-all duration-300 ease-in-out animate-slide-in-right-widget">
      <header className="p-3.5 border-b border-brand-border/50 flex justify-between items-center bg-brand-surface-alt/30 rounded-t-xl">
        ...
        <button
          onClick={onClose}
          className="p-1 rounded-full hover:bg-brand-surface-alt text-brand-text-muted hover:text-brand-text transition-colors"
      ...
      {!currentUser &&
        apiKeyExists &&
        !(!currentUser?.is_pro_user && apiKeyExists) && ( // Show login prompt if not showing upgrade prompt
      ...
      <form
        ...
        className="p-2.5 border-t border-brand-border/50"
      >
