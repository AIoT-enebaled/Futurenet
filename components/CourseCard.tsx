  ...
  return (
    <div className="bg-brand-surface rounded-xl shadow-xl overflow-hidden transition-all duration-300 ease-in-out hover:shadow-glow-cyan transform hover:-translate-y-1.5 border border-brand-border/30 hover:border-brand-cyan/40 flex flex-col h-full">
      <Link to={`/learning/courses/${course.id}`} className="block group">
        <div className="relative">
          <img
            ...
            src={
              course.imageUrl ||
              "https://picsum.photos/seed/courseplaceholder/400/225"
            }
            alt={course.title}
          />
          {course.isProCourse && (
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-grow">
        {primaryCategory && (
          <div className="flex items-center mb-1.5">
            {primaryCategory.icon && (
              <primaryCategory.icon className="w-3.5 h-3.5 mr-1.5 text-brand-cyan flex-shrink-0" />
            )}
        ...
        >
          <h3
            className="text-md sm:text-lg font-semibold text-brand-text group-hover:text-brand-purple transition-colors line-clamp-2"
            title={course.title}
          >
            {course.title}
          </h3>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-brand-text-muted mb-3">
          <span className="flex items-center" title="Difficulty">
            <BarChart3Icon className="w-3.5 h-3.5 mr-1 text-brand-text-darker" />{" "}
            {course.difficulty}
          </span>
          <span className="flex items-center" title="Duration">
            <GraduationCapIcon className="w-3.5 h-3.5 mr-1 text-brand-text-darker" />{" "}
            {course.durationHours} hrs
          </span>
          {course.rating && (
            <span className="flex items-center" title="Rating">
              <StarIcon className="w-3.5 h-3.5 mr-1 text-yellow-400 fill-current" />{" "}
